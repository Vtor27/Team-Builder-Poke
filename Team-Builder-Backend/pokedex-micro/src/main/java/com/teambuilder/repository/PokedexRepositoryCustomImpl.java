...
public class PokedexRepositoryCustomImpl implements PokedexRepositoryCustom{
	...
	public List<Pokemon> advancedFilter(String name, String type, String regionApparitions, Integer generation, String tier, String regionOrigin){
		...
		//Fetch a todos los campos para que traiga la info completa de los pokes filtrados aunque no se filtre por algún campo
		root.fetch("types", JoinType.LEFT);
		...
		if (name != null && !name.isEmpty()) {
			// Filtro por nombre (transforma todoaminusculas), buscando coincidencias parciales con LIKE: 
		...
		if (type != null && !type.isEmpty()) {
			Join<Pokemon, Types> joinTypes = root.join("types", JoinType.INNER);
			filters.add(cb.equal(cb.lower(joinTypes.get("name")), type.toLowerCase()));	//name en este casoes el nombre del tipo(está en la tabla type)
		}
		...
		// Convierte la lista de predicados a un array y los combina con AND para que se cumplan todos los criterios.
